...
| Score validation | `nullable` · `numeric` (rejects `NaN` / `Infinity` / text) · `min:0` (no negatives) · `decimal:0,2` · `<= max_score` (server-side vs the parent); `max_score` is frozen once any score is recorded — no change up or down (tested both directions) |
