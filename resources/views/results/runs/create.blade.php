@php
    ...
    $ready = $sessions->isNotEmpty() && $levels->isNotEmpty() && $gradingSchemes->isNotEmpty() && $weightingSchemes->isNotEmpty();
@endphp

<x-layouts.authenticated :title="__('New result run')">
    <div class="max-w-2xl space-y-6">
        <p class="text-sm">
            <a href="{{ route('results.runs.index') }}" class="text-brand-600 hover:text-brand-700">{{ __('← Result runs') }}</a>
        </p>

        <x-card :title="__('Result run details')">
            @unless ($ready)
                <x-empty-state :title="__('Setup needed')"
                    :description="__('You need an academic session, a class, an active grading scheme and an active weighting scheme before creating a result run.')">
                    <x-slot:actions>
            ...
            @else
                <form method="POST" action="{{ route('results.runs.store') }}"
                    x-data="{
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div class="space-y-1">
                            <label for="grading_scheme_id" class="block text-sm font-medium text-gray-700">{{ __('Grading scheme') }} <span class="text-red-500">*</span></label>
                            <select id="grading_scheme_id" name="grading_scheme_id" class="{{ $selectClass }}">
                                <option value="">{{ __('Select a grading scheme') }}</option>
                                @foreach ($gradingSchemes as $scheme)
                                    <option value="{{ $scheme->id }}" @selected(old('grading_scheme_id') == $scheme->id)>{{ $scheme->name }}</option>
                                @endforeach
                            </select>
                            @error('grading_scheme_id') <p class="text-xs text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <div class="space-y-1">
                            <label for="result_weighting_scheme_id" class="block text-sm font-medium text-gray-700">{{ __('Weighting scheme') }} <span class="text-red-500">*</span></label>
                            <select id="result_weighting_scheme_id" name="result_weighting_scheme_id" class="{{ $selectClass }}">
                                <option value="">{{ __('Select a weighting scheme') }}</option>
                                @foreach ($weightingSchemes as $scheme)
                                    <option value="{{ $scheme->id }}" @selected(old('result_weighting_scheme_id') == $scheme->id)>{{ $scheme->name }} ({{ rtrim(rtrim(number_format($scheme->totalWeight(), 2), '0'), '.') }}%)</option>
                                @endforeach
                            </select>
                            @error('result_weighting_scheme_id') <p class="text-xs text-red-600">{{ $message }}</p> @enderror
                        </div>
                    </div>

                    <x-checkbox name="ranking_enabled" :label="__('Calculate class position')" :checked="old('ranking_enabled', true)" />

                    <div class="flex items-center gap-2 pt-2">
                        <x-button type="submit">{{ __('Create result run') }}</x-button>
                        <x-button :href="route('results.runs.index')" variant="ghost">{{ __('Cancel') }}</x-button>
                    </div>
                    <p class="text-xs text-gray-400">{{ __('Compilation only ever uses locked assessment scores for this exact class and term.') }}</p>
                </form>
            @endunless
        </x-card>
    </div>
