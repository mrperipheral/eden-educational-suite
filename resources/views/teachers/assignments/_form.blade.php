@php
    ...
    $oldSession = old('academic_session_id', $assignment->academic_session_id);
    $oldPeriod = old('academic_period_id', $assignment->academic_period_id);
    $oldLevel = old('academic_level_id', $assignment->academic_level_id);
    $oldArm = old('level_arm_id', $assignment->level_arm_id);
@endphp
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div class="space-y-1">
            ...
            <select id="status" name="status" class="{{ $selectClass }}">
                @foreach (\App\Enums\TeacherAssignmentStatus::all() as $s)
                    <option value="{{ $s->value }}" @selected(old('status', $assignment->status?->value ?? 'active') === $s->value)>{{ $s->label() }}</option>
                @endforeach
            </select>
            @error('status') <p class="text-xs text-red-600">{{ $message }}</p> @enderror
        </div>
        <x-input name="started_on" type="date" :label="__('Start date')"
            :value="old('started_on', $assignment->started_on?->toDateString())" required />
        <x-input name="ended_on" type="date" :label="__('End date')"
            :value="old('ended_on', $assignment->ended_on?->toDateString())" :hint="__('Required when ended.')" />
    </div>
</div>
